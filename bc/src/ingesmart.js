 ...
 * - Pour le Master, une soutenance notée à 10 minimum doit être validée.
...
class DiplomaContract extends Contract {
     ...
     */
    async checkEligibility(ctx, studentId, level, ...years) {
    ...
    /**
     * Soumission du mémoire par l'étudiant (Master uniquement)
     */
    async submitThesis(ctx, studentId, thesisHash) {
        ...
        return `Mémoire soumis pour l'étudiant ${studentId}.`; // Confirme la soumission
    }

    /**
     * Validation ou refus du mémoire par l'enseignant (Master)
     */
    async validateThesis(ctx, studentId, thesisDecision, defenseGrade) {
        const student = await ctx.stub.getState(studentId); // Récupère les données de l'étudiant
        if (!student || student.length === 0) { // Vérifie si l'étudiant existe
            throw new Error(`L'étudiant ${studentId} n'existe pas.`);
        }
        ...
        const studentData = JSON.parse(student.toString()); // Convertit les données en JSON
        if (!studentData.thesisHash) { // Vérifie si le mémoire a été soumis
            throw new Error(`Mémoire non soumis pour l'étudiant ${studentId}.`);
        }

        if (!['accept', 'reject'].includes(thesisDecision)) { // Vérifie que la décision est valide
        ...
        return `Mémoire ${thesisDecision === 'accept' ? 'validé' : 'refusé'}, note de soutenance ${defenseGrade} pour l'étudiant ${studentId}.`;
    }
}
